...
這套準備工具的目的，是在原作者更新抽獎活動或店家連結時，先驗證相容性並產生可以檢查的新版；只有提交到 GitHub 的 `main` 後，已上線網站才會改變。
...
- `custom/continuous-draw.js`：跳過店家、復原、縣市／商品／時間篩選、店家開始時間判斷、全自動接力與 0.7 秒停止區等行為
...
- 時間篩選會限制連續抽選只處理相同開始時間的店家；尚未到店家開始時間時，不會提前開啟 LINE 抽選。
...
工具會檢查抽獎頁、操作模式區、縣市篩選器、店家清單與原始連續抽選程式的標記，也會確認 `.draw-store`、`.draw-item` 資料仍然存在。抽獎網址同時支援舊版 `.draw-link[href]` 與新版商品列 `data-draw-href`；若新版同時保留兩種格式，會以商品列網址為準，避免重複計算。這些條件任何一項改變時，更新都會安全停止，避免把壞掉的版本推上線。
